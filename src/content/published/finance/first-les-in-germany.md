---
title: Your first LES in Germany: reading the paycheck
summary: >-
  Your first LES after PCS hits with OHA, COLA, TLA, and possibly a travel voucher: all USD, all untaxed. Expect confusion; expect 30–60 days for it to stabilize. File your travel voucher within 5
  days of arrival.
topic: finance
phase: arrive
usag: wiesbaden
profile_tags:
  status:
    - soldier
    - daciv
sources:
  - tier: T1
    url: https://myarmybenefits.us.army.mil/Benefit-Library
    label: MyArmyBenefits: OHA
    score:
      authority: 5
      currency: 5
      accuracy: 5
      purpose: 5
      license: 5
  - tier: T1
    url: https://myarmybenefits.us.army.mil/Benefit-Library
    label: MyArmyBenefits: COLA
    score:
      authority: 5
      currency: 5
      accuracy: 5
      purpose: 5
      license: 5
  - tier: T2
    url: https://installations.militaryonesource.mil/military-installation/usag-wiesbaden/military-and-family-support-center/financial-assistance
    label: MOS: Wiesbaden Financial Assistance
    score:
      authority: 4
      currency: 5
      accuracy: 5
      purpose: 3
      license: 5
last_verified: '2026-04-16'
poc_volatile: false
checklist: true
order: 30
---

## What

Your Leave and Earnings Statement (LES) changes substantially when you PCS to Wiesbaden. New lines appear (OHA, COLA, TLA), old ones may disappear (US BAH, state tax withholding), and the whole thing takes 1-3 LES cycles to stabilize.

Here's what you'll see and what it means.

## New lines on your LES

### Entitlements (money you earn)

**OHA; Overseas Housing Allowance**
- Replaces BAH
- Shown as "OVSHSG ALLOW" or "OHA" on LES
- Paid monthly at your rank's Wiesbaden rate, capped at your actual rent
- **Not taxable**
- Appears once your first HSO-approved lease is entered into DMDC

**COLA; Cost of Living Allowance**
- Daily stipend reflecting local cost-of-living vs. CONUS
- Shown as "OS COLA" or "CONUS COLA" (different)
- Paid with every LES, fluctuates with EUR/USD exchange rate
- **Not taxable**

**TLA; Temporary Lodging Allowance**
- Paid for the gap between arrival and permanent quarters
- Appears in the "entitlements" section during the TLA window
- Lodging portion (up to per-diem cap) + meals per-diem
- **Not taxable**
- Stops when you move into permanent quarters

**MIHA; Moving-In Housing Allowance**
- One-time payment when you set up a first German off-post residence
- Covers setup costs (security deposit, first-time utility hookups, etc.)
- Paid once per OHA cycle (per tour, typically)
- Shown as "MIHA"; filed via Finance

**PCS travel voucher**
- Separate from LES, but deposits into your pay account
- Covers mileage, per-diem, lodging during PCS travel
- File within **5 business days of arrival** via DTS online
- Reimbursement typically 10-15 business days after filing

### Deductions (money removed)

**Federal tax withholding**
- Still applies; your salary is federally taxable regardless of where you live
- Adjust W-4 if the first few LES over- or under-withhold

**State tax withholding**
- Depends on your Home of Record and Legal Residency
- If you changed SLR to a no-income-tax state (TX, FL, WA, NV, etc.), no state withholding
- If you kept SLR in a tax state (VA, CA, NC, etc.), state tax continues
- Check with Legal Office about SLR options

**SGLI / FSGLI / Family SGLI**
- Continues; shown the same as CONUS

**TSP contribution**
- Continues

**SSN (Social Security + Medicare)**
- Withheld as normal

### What you'll NOT see

- **German income tax**: SOFA-status personnel on DoD source income are exempt
- **Kirchensteuer (church tax)**: exempt under SOFA
- **Kfz-Steuer (vehicle tax)**: exempt for USAREUR-plated POVs
- **German social insurance**: not withheld for SOFA personnel

## Typical timeline

### LES #1 after arrival (30 days in)
- **OHA**: probably $0 (lease not yet approved / processed)
- **COLA**: starts appearing after sign-in
- **TLA**: probably present
- **Travel voucher**: maybe not yet (depends on filing timing)
- **You might see a DFAS message** about pending allowances

### LES #2 (60 days in)
- **OHA**: kicks in if you've moved to permanent housing and HSO has the lease
- **COLA**: stabilized
- **TLA**: should have stopped
- **Travel voucher**: should be deposited
- **MIHA**: appears if applicable

### LES #3 (90 days in)
- Everything should be clean
- Recurring monthly pattern established

## How to sanity-check

Your total take-home should roughly equal:
- Base pay + OHA + COLA + any other entitlements
- **Minus**: federal tax, state tax (if any), SGLI, TSP, dependent care deductions, any voluntary deductions

If your first-month take-home is dramatically off from expected, visit Finance on Clay Kaserne **before** the next payday.

## Managing Finance surprises

### Under-payment (less than expected)
- **OHA not yet flowing**: verify lease is approved by HSO
- **COLA not calculated**: verify you've signed in as "command-sponsored, accompanied" if applicable
- **Missing allowance codes**: your unit's S1 or DA Civ HR checks the PCS in-processing paperwork

### Over-payment (more than expected)
- Happens. DFAS catches it eventually and recoups via future LES.
- **Don't spend** a windfall from over-payment; save until the LES stabilizes.

### Travel voucher issues
- DTS often flags errors; respond within the prompt window
- Missing receipts delay processing; scan everything within 5 days of arrival
- Large receipts (Army Lodge total, rental car, etc.) are essential

## Wiesbaden-specific gotchas

- **Your first OHA won't cover the first month's rent + deposit**; OHA is a monthly reimbursement, not a lump sum. You out-of-pocket the deposit initially.
- **COLA lags the market**; if EUR strengthens suddenly, your COLA won't catch up for 1-2 months
- **MIHA is claim-based**; you file receipts with Finance for reimbursement; it's not automatic
- **Rundfunkbeitrag is not reimbursed**; €18.36/month German TV tax is on you for the duration

## Financial checklist

### Week 1
- [ ] Visit Finance for PCS in-processing brief
- [ ] Confirm DEERS dependent status matches entitlement assumptions
- [ ] File travel voucher in DTS

### Week 2
- [ ] Receive first LES post-arrival (check for TLA, COLA, initial OHA)
- [ ] Start saving Army Lodge receipts for TLA claim
- [ ] Start German bank account setup

### Week 3-4
- [ ] Submit HSO-approved lease for OHA initiation
- [ ] Reconcile first LES
- [ ] Set up auto-transfer to German bank for rent

### Month 2
- [ ] OHA flowing, TLA stopped
- [ ] File MIHA if applicable (first off-post setup)
- [ ] Stabilize budget

### Month 3+
- [ ] LES pattern established
- [ ] Monthly review: COLA trend, OHA adequacy, savings rate

## Financial Readiness Program (FRP) at ACS

The **Financial Readiness Program** at ACS is free 1:1 budget counseling. If any of the above is confusing or if you're hitting the transition-month cash crunch, book an FRP appointment:

- **DSN 548-9201** → ask for FRP
- Multi-session engagement available
- No-judgment environment; they're used to PCS complications

## Deductions and voluntary contributions to revisit

- **W-4**: update for SOFA-exempt German tax status (check box for foreign residency if asked)
- **TSP**: consider increasing contribution; lower cost-of-living in Wiesbaden relative to DC/NYC often allows higher saving
- **Leave policy**: track leave accrual + use-or-lose thresholds
- **Retirement**: PCS-out LES will zero out OHA/COLA; plan return-to-CONUS budget accordingly

## Gotchas

- **Don't compare Wiesbaden LES to Fort X LES line-by-line.** The entitlement structure is different; total take-home may be more or less depending on rank + family + specific rates that month.
- **Foreign-currency fluctuations affect COLA**; a cheap-Euro month may mean COLA drops
- **Finance office walk-ins take time**; book via the unit S1 or email ahead
- **Commercial landlords expect €0 OHA involvement**; you pay them in EUR; OHA comes to you in USD separately
- **Keep all Army Lodge receipts** for TLA and travel voucher
- **TurboTax / H&R Block** work for Active Duty overseas; your LES data is the same
- **VITA tax prep** at Legal Office (free, Jan-Apr); can help with SOFA-status implications

## Source notes

Synthesized from MyArmyBenefits OHA/COLA overviews, MilitaryOneSource's financial assistance overview for Wiesbaden, and community knowledge from active-duty Soldiers' first 90-day experiences. Specific rates change quarterly; always verify current OHA and COLA rates via the Defense Travel per diem calculator.
