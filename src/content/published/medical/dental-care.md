---
title: Dental care — on-post for AD, off-post for everyone else
summary: Active Duty use the Wiesbaden Dental Clinic (DSN 590-1515). Dependents enroll in TRICARE Dental Program (TDP) and see off-post German dentists. TDP coverage is strong; German dentistry is excellent.
topic: medical
phase: life
usag: wiesbaden
profile_tags:
  status:
    - soldier
    - daciv
    - family
sources:
  - tier: T1
    url: https://home.army.mil/wiesbaden/5417/1396/5081/2024_WELCOME_PACKET.pdf
    label: 2024 Welcome Packet (PDF)
    score:
      authority: 5
      currency: 5
      accuracy: 5
      purpose: 5
      license: 5
  - tier: T2
    url: https://installations.militaryonesource.mil/military-installation/usag-wiesbaden/health/health-care
    label: MOS — Wiesbaden Health Care
    score:
      authority: 4
      currency: 5
      accuracy: 5
      purpose: 3
      license: 5
last_verified: '2026-04-16'
poc_volatile: true
order: 40
poc:
  - name: Wiesbaden Dental Clinic
    dsn: 590-1515
    phone: +49-6371-9464-1515
    address: Clay Kaserne, Bldg 1040 complex, Wiesbaden
    maps_query: Clay Kaserne Building 1040 Wiesbaden
    hours: Mon–Fri business hours; call for current appointment availability
---

## What

Dental in Germany under TRICARE splits two ways:

- **Active Duty** Soldiers use the **Wiesbaden Dental Clinic** on Clay Kaserne; free, routine + some complex, co-located with Army Health Clinic.
- **Dependents and DA Civilians** use **off-post German dentists**, paying via TRICARE Dental Program (TDP) or out-of-pocket.

TDP is NOT automatic; dependents must **enroll** separately from TRICARE medical. Do this in your first 30 days.

## Active Duty; Wiesbaden Dental Clinic

### Where
**Clay Kaserne**, within the Army Health Clinic complex (Bldg 1040 area).

### Services
- Routine dental exams and cleanings
- Fillings and restorations
- Oral surgery (basic)
- Root canals (some cases; complex go off-post)
- Periodontal care
- Dental readiness classifications (Class I/II/III)

### Readiness is mandatory
Active Duty Soldiers are evaluated annually for deployment readiness. Class I = healthy; Class II = minor work needed; Class III = major work, not deployment-ready. Plan your annual exam around PCS peaks to avoid readiness flags.

### What's NOT at ARHC Dental
- Orthodontics (referred out)
- Cosmetic (veneers, etc.)
- Complex implants

## Dependents; TRICARE Dental Program (TDP)

### Enrollment
TDP is **Delta Dental** overseas. Enrollment happens:
1. Within 30 days of PCS arrival
2. Via milConnect → Benefits → Add TRICARE Dental
3. Monthly premium charged to sponsor's LES
4. Effective date: 1st of month after enrollment

### Cost
- **Single member** (one dependent): ~$12/mo
- **Family** (multiple dependents): ~$35-40/mo
- Discounts for ADFMs vs. retirees/reservists

### What TDP covers
- Routine exams and cleanings (twice yearly, covered at 100%)
- Fillings (80-100%)
- Extractions
- Root canals (varies)
- Crowns (varies, after initial 12-month waiting period)
- Orthodontics (after waiting period; kids + some adults; $2,000 lifetime cap)

### What TDP doesn't cover
- Cosmetic (whitening, veneers for appearance)
- Implants in full (partial coverage)
- Most dental work in the first 12 months for "major" categories

### How to use TDP off-post
1. Find a dentist in Wiesbaden that accepts TDP (not all do)
2. Book appointment
3. At appointment, present TDP insurance card + dependent ID
4. Dentist bills Delta Dental directly OR you pay and file for reimbursement

## Off-post German dentists

### What to expect
- **High quality**: German dental training is rigorous; modern practices, excellent tech
- **English-speaking**: Many Wiesbaden dentists have English-speaking staff
- **Zuzahlung** (patient co-pay): 10-30% typical for things TDP partially covers
- **Long appointments**: German dentists spend more time than US norm
- **Gentle approach**: German dentistry trends gentler than US; less aggressive drilling

### Finding a TDP-accepting dentist
1. Delta Dental's overseas directory (phone/web)
2. Community recommendations via USAG Wiesbaden Facebook groups
3. Ask your sponsor or the ARHC Dental clinic for referrals

### Typical visit costs (Wiesbaden area)
- **Routine exam + cleaning**: €60-€120 (TDP covers most)
- **Filling**: €60-€200 depending on complexity (TDP covers 80-100%)
- **Root canal**: €400-€900 (TDP partial, you pay difference)
- **Crown**: €800-€1,500 (TDP partial after waiting period)
- **Orthodontics (child, full treatment)**: €3,000-€6,000 (TDP max $2,000 lifetime)

### Paying direct vs. claim
Some dentists invoice Delta Dental directly; others want you to pay and claim. Ask at booking.

## Emergency dental

### After-hours / weekend
- **Not** at the Wiesbaden Dental Clinic (business hours only)
- **German dental emergencies**: call 116 117 (medical on-call line)
- **Tooth trauma**: go to hospital ER (HSK, Asklepios) if severe

### Rooted-out tooth
- Place tooth in milk or saliva
- Go to HSK ER immediately
- Dental surgeon on-call can re-implant within 30-60 min window

## Kids' dental

### First dental visit
German dentists often begin seeing kids at age 1-2, compared to US norm of age 3-4. Preventive approach.

### Orthodontics
- TDP covers with $2,000 lifetime cap per child
- German orthodontists excellent; treatment paths similar to US
- **Stress**: braces are a 2+ year commitment; coordinate with PCS timing

### Sealants + fluoride
- Standard German pediatric dentistry
- TDP covers

## Active Duty; specific readiness notes

### Before PCS
- Complete readiness exam + any major work
- Leave CONUS in Class I

### In Wiesbaden
- Annual readiness exam at Wiesbaden Dental Clinic
- Don't let a deployment catch you in Class III

### Emergency while deployed
- Military dental via deployed channels

## Gotchas

- **TDP enrollment ≠ automatic.** You must actively enroll within 30 days of PCS.
- **TDP has waiting periods.** Major work (crowns, orthodontics) subject to 12-month waiting period for new enrollees.
- **Not all Wiesbaden dentists take TDP.** Confirm before booking.
- **Dental emergencies** aren't covered by TRICARE medical; they're TDP.
- **German dental translations**: procedure names differ. Ask dentist for English-language breakdown of the quote.
- **DA Civilian dental**: covered by FEDVIP (separate federal dental program), not TDP. Different enrollment path.
- **Retirees** use TRICARE Dental Retiree option; also separate from active-duty TDP.
- **US-based orthodontic in-progress** treatments can continue with German orthodontist, but sometimes you pay out-of-pocket during the transfer.
- **Dental X-rays**: German dentists use them less than US. TDP covers less frequent X-rays. This is per German dental guidelines.
- **Veneers + cosmetic** work: rarely covered by TDP. Germans less into cosmetic dentistry than US; fewer practices offer it.

## Pro tips

- **Book your first cleaning** 30-60 days after arrival; don't wait until you need dental work
- **Build a relationship** with one local dentist for ongoing care
- **Keep all receipts** for reimbursement; German dental invoices are detailed
- **If TDP is denied** for something: appeal; often reversed with proper documentation
- **Compare quotes** from 2-3 dentists for major work (root canals, crowns); prices vary
- **Consider OCONUS dental tourism**: some families go to Hungary/Poland for extensive work; often 30-50% cheaper and high quality. Your TDP may or may not reimburse.

## Source notes

Synthesized from the Wiesbaden Welcome Packet dental section and MilitaryOneSource's Wiesbaden health-care overview. TDP enrollment rules and coverage percentages updated by Delta Dental annually; verify current policies at tricare-dental.mil before enrolling.
